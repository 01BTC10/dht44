...
do_add_mapping(void)
{
    ...
    int rc = UPNP_AddPortMapping(
        ...
        "libbep44",                    /* description */
        "UDP",                      /* protocol */
        NULL,                       /* remoteHost */
        s.lease_str                 /* leaseDuration ("0" = permanent) */
    );
    if (rc != UPNPCOMMAND_SUCCESS) {
        fprintf(stderr, "[libbep44:upnp] AddPortMapping(%s/UDP) failed: %s\n",
...
{
    if (s.ready) {
        fprintf(stderr, "[libbep44:upnp] already initialised\n");
        return -1;
    }
    ...
    if (!devlist) {
        fprintf(stderr, "[libbep44:upnp] no IGD discovered (err=%d)\n", discover_err);
        return -1;
    }
    ...
    if (rc != UPNP_CONNECTED_IGD) {
        fprintf(stderr, "[libbep44:upnp] no usable IGD (rc=%d)\n", rc);
        FreeUPNPUrls(&s.urls);
        return -1;
    }
    ...
    fprintf(stderr, "[libbep44:upnp] mapped %s/UDP on IGD (lease=%us, %s -> %s)\n",
...
upnp_shutdown(void)
{
    ...
    int rc = UPNP_DeletePortMapping(s.urls.controlURL,
                                    ...
                                    s.port_str, "UDP", NULL);
    if (rc != UPNPCOMMAND_SUCCESS) {
        fprintf(stderr, "[libbep44:upnp] DeletePortMapping(%s/UDP): %s\n",
                s.port_str, strupnperror(rc));
    } else {
        fprintf(stderr, "[libbep44:upnp] removed %s/UDP mapping\n", s.port_str);
    }
    FreeUPNPUrls(&s.urls);
    s.ready = 0;
